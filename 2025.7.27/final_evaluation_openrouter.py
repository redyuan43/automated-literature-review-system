...
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "") # Get API key from environment or empty string as fallback
...
def evaluate_technical_depth_with_llm(text):
    """
    Use OpenRouter (DeepSeek) to evaluate technical depth.
    Returns a dictionary with 'score' and 'justification'.
    """
    try:
...
Consider:
        ...
        6. Use a numeric accuracy score between 0.00 and 1.00 for each claim.
        ...
        Scoring guidance:
        - 0.90-1.00: ONLY for text with exceptional technical depth, equivalent to expert-level scientific publications
        - 0.70-0.80: Strong technical depth with advanced concepts, but not at publishable research level
        - 0.50-0.60: Moderate technical depth, appropriate for educated professionals but not specialists
        - 0.30-0.40: Basic technical content with some field-specific terminology
        - 0.00-0.20: Minimal technical content, mainly general knowledge
        ...
        if match:
            ...
            return {
                "score": 0.5,
                "justification": "Failed to parse JSON from model output 1"
            }
...
def evaluate_clarity_with_llm(text):
    ...
    try:
        ...
        Provide a score from 0.00-1.00 and a brief justification.
        ...
        if match:
            ...
            return {
                "score": 0.5,
                "justification": "Failed to parse JSON from model output 2"
            }
...
def evaluate_structure_with_llm(text):
    """
    Use OpenRouter (DeepSeek) to evaluate the structural organization of the text.
    ...
    try:
        ...
        if match:
            ...
            return {
                "score": 0.5,
                "justification": "Failed to parse JSON from model output 3"
            }
...
5. Use a numeric accuracy score between 0.0 and 1.0 for each claim.
6. You must output ONLY a valid JSON object with the following structure:

Do not include any explanation or markdown. Only output the JSON object.

Format your response as a JSON object with this structure:
{{
    "score": 0.85,  # Overall factual accuracy score
    "analysis": [
        {{
            "claim": "specific claim or statement from text",
            "citation_id": "[X]",  # The exact citation ID used in the text
            "accuracy": 0.92,  # Accuracy score for this claim
...
Scoring criteria:
- 0.90-1.00: All claims are fully supported by their corresponding references
- 0.70-0.89: Most claims are accurate with minor discrepancies
- 0.40-0.69: Some claims are accurate but others lack support
- 0.10-0.39: Many claims lack support or contradict references
- 0.00-0.09: No claims are supported by the references"""
